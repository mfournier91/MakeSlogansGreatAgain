...
class GenerateContainer extends Component {
  constructor(props){
    ...
    this.state.url = {content: "http://twitter.com/intent/tweet?text=Make" + this.state.noun.content + this.state.adj.content + "Again%20%23MakeSlogansGreatAgain%20http://bit.ly/2cd3NUL"};
  ...
  submitNounQuery(evt){ //function invoked by clicking on button in the Generate component
      ...
      queryNounOnly().then(data => { //See Utils.js to see this function.
        ...
        this.setState({
        url : {content: "http://twitter.com/intent/tweet?text=Make" + this.state.noun.content + this.state.adj.content + "Again%20%23MakeSlogansGreatAgain%20http://bit.ly/2cd3NUL"}
      });
      });
  }
